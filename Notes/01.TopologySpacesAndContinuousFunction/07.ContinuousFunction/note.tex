...
\begin{definition}[continuous]\label{def:Continuous}\footnote{
      As the continuity of a function is different as the topological spaces are different. So if we want to emphasis this fact, we say that \mt{f} is continuous \defineNewWorld{relative}\label{def:ContinuousRelativeTo} to specific topologies on \mtb{X} and \mtb{Y}.
}
      Let \mtb{X} and \mtb{Y} be topological spaces. A function \mt{f: \mathbb{X}\rightarrow \mathbb{Y}} is said to be \defineNewWorld{continuous} if for each open subset \mt{V} of \tmb{Y}, the set \mt{f^{-1}(V)} is an open subset of \mtb{X}.
\end{definition}
...
\begin{theorem}[Maps into products]\label{theorem:MapsIntoProducts} \footnote{
      The map \mt{f_{1},f_{2}} are called the \defineNewWorld{coordinate functions}\label{def:CoordinateFunctions} of \mt{f}
}
